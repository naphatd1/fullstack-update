...
const ApiStatusToggle: React.FC<ApiStatusToggleProps> = ({ className = '', compact = false }) => {
  ...
  const checkApiStatus = async () => {
    ...
    console.log('🔍 API Status Check - URL:', `${API_URL}/health`);
    console.log('🔍 API Status Check - Full API_URL:', API_URL);
    console.log('🔍 API Status Check - ENV:', process.env.NEXT_PUBLIC_API_URL);
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('❌ API Status Check - FAILED:', response.status, response.statusText);
        setIsOnline(false);
