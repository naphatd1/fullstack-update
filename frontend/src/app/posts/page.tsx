...
import { FileText, Plus, Edit, Trash2, Eye, EyeOff, Home } from 'lucide-react';
...
const PostsPage: React.FC = () => {
  ...
  const handleCreatePost = async (postData: { title: string; content: string; images: string[] }) => {
    try {
      console.log('Creating post:', postData);

      // สร้างโพสต์ data สำหรับแสดงผล
      const newPost = {
      ...
      // พยายามส่งไปยัง backend ในพื้นหลัง
      setTimeout(async () => {
        try {
    ...
    queryFn: async () => {
      try {
        ...
        if (isBackendAvailable) {
          ...
          // รวมข้อมูลจาก localStorage
          const userPosts = JSON.parse(localStorage.getItem('userPosts') || '[]');
          const allPosts = [...userPosts, ...(data || [])];
          
          return { data: allPosts };
        } else {
          // Use fallback posts + user posts
          console.log('Using fallback posts (demo mode)');
          const fallbackData = await fallbackPosts.getMyPosts();
          const userPosts = JSON.parse(localStorage.getItem('userPosts') || '[]');
          const allPosts = [...userPosts, ...(fallbackData.data || [])];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-4 sm:space-y-0">
        <div>
          <h1 className="text-responsive-title text-gray-900 dark:text-white transition-colors duration-200">โพสต์ทั้งหมด</h1>
          <p className="text-responsive-body text-gray-600 dark:text-gray-300 transition-colors duration-200">ดูประกาศบ้านและอสังหาริมทรัพย์</p>
        </div>
        <div className="flex space-x-2">
          <Link
            href="/sell-house"
            className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 sm:px-4 sm:py-2 rounded-lg transition-colors duration-200 flex items-center"
          >
            <Home className="h-3 w-3 sm:h-4 sm:w-4 mr-1.5 sm:mr-2" />
            <span className="text-xs sm:text-sm">ขายบ้าน</span>
          </Link>
          <button
          ...
          >
            ...
            <span className="text-xs sm:text-sm">สร้างโพสต์</span>
          </button>
        </div>
      </div>
