...
// ตัวเลือกสำหรับปิด notification
const SHOW_FALLBACK_NOTIFICATION = process.env.NODE_ENV === 'development';

export const showFallbackNotification = () => {
  // ปิด notification ไว้ก่อน เพื่อไม่ให้รบกวนผู้ใช้
  // สามารถเปิดได้โดยเปลี่ยน false เป็น true
  const ENABLE_NOTIFICATION = false;
  
  if (!fallbackNotificationShown && ENABLE_NOTIFICATION) {
    fallbackNotificationShown = true;
    toast.success(
      '🏠 ระบบทำงานในโหมดทดสอบ - ข้อมูลตัวอย่างพร้อมใช้งาน',
      {
        duration: 2000,
        position: 'top-right',
        style: {
